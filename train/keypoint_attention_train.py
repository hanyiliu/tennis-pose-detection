...
def compute_weighted_loss(outputs, target_heatmaps, criterion, aux_weights):
	if isinstance(outputs, tuple):
		main, aux1, aux2, aux3 = outputs
		return (
			criterion(main, target_heatmaps)
			+ aux_weights[0] * criterion(aux1, target_heatmaps)
			+ aux_weights[1] * criterion(aux2, target_heatmaps)
			+ aux_weights[2] * criterion(aux3, target_heatmaps)
		), main

	return criterion(outputs, target_heatmaps), outputs


@torch.no_grad()
def evaluate(model, loader, criterion, device, aux_weights):
	model.eval()
	total_loss = 0.0
	total_count = 0
	...
	for images, target_heatmaps in loader:
		...
		loss, pred_heatmaps = compute_weighted_loss(outputs, target_heatmaps, criterion, aux_weights)
...
def train_keypoint_attention_model(args):
	...
	if args.image_height % 32 != 0 or args.image_width % 32 != 0:
		raise ValueError(
			"image_height and image_width must be divisible by 32 for KeypointAttentionDetectionModel. "
	...
	model = KeypointAttentionDetectionModel(num_keypoints=args.num_keypoints).to(device)
	...
	_, pred0 = compute_weighted_loss(
		outputs0,
		heatmaps0,
		criterion,
		aux_weights=(args.aux_weight1, args.aux_weight2, args.aux_weight3),
	)
	...
	best_path = "checkpoints/keypoint_attention_best.pt"
	deploy_weights_path = os.path.join(args.export_dir, "keypoint_attention_best_state_dict.pt")
	deploy_torchscript_path = os.path.join(args.export_dir, "keypoint_attention_best_torchscript.pt")
	...
	if args.resume_from is not None:
		...
		try:
			...
			raise RuntimeError(
				...
				"Use a training checkpoint/state_dict path such as checkpoints/keypoint_attention_best.pt "
				"or exports/keypoint_attention_best_state_dict.pt, not exports/keypoint_attention_best_torchscript.pt. "
	...
	for epoch in range(1, args.epochs + 1):
		...
		val_loss, cv_acc = evaluate(model, val_loader, criterion, device, aux_weights)
	...
	test_loss, test_acc = evaluate(model, test_loader, criterion, device, aux_weights)
	...
	model_cpu = KeypointAttentionDetectionModel(num_keypoints=args.num_keypoints)
...
def main():
	...
	parser.add_argument("--aux_weight1", type=float, default=0.4)
	parser.add_argument("--aux_weight2", type=float, default=0.2)
	parser.add_argument("--aux_weight3", type=float, default=0.1)

	parser.add_argument("--resume_from", type=str, default=None)
	parser.add_argument("--export_dir", type=str, default="exports")

	args = parser.parse_args()
	train_keypoint_attention_model(args)
