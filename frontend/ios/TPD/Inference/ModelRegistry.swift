...
/// assumed in Swift, because each is silent when wrong: `output.type` (a second softmax keeps
/// the argmax but flattens every confidence), `labels` (per model — the pose-split ResNet puts
/// forehand at 0 where the others put backhand), and `labelOrder.status`, which the picker must
/// show. Branch on those, never on `id`.
struct TPDModelEntry: Decodable, Sendable, Equatable, Identifiable {
    ...
    /// plain 0–255 RGB and the registry's mean/std are documentation, left undecoded.
    struct Input: Decodable, Sendable, Equatable {
        let name: String, size: Int, normalizationBakedIn: Bool
    }
    ...
    struct Build: Decodable, Sendable, Equatable { let pipelineSpec: String? }
    ...
    /// registry, and a test drives the trap without loading Core ML.
    func result(from raw: [Float], frameSize: CGSize,
        ...
        let probabilities = output.type == .logits ? EngineIO.softmax(raw) : raw
        var best = -Float.infinity, bestIndex = -1
        for (index, value) in probabilities.enumerated() where value > best {
...
struct ModelRegistry: Decodable, Sendable, Equatable {
    ...
    func first(of kind: TPDModelEntry.Kind) throws -> TPDModelEntry {
        guard let entry = models.first(where: { $0.kind == kind }) else {
            throw Self.malformed("lists no \(kind.rawValue) model; have \(models.map(\.id))")
        }
        return entry
    }
    ...
    static func decode(_ data: Data) throws -> ModelRegistry {
        ...
        guard registry.schemaVersion == supportedSchemaVersion else {
            throw malformed("schemaVersion \(registry.schemaVersion), not "
                            + "\(supportedSchemaVersion) — this build cannot read it")
        }
